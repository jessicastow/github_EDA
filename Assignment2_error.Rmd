...
```{r}
df_earthquakes <- st_as_sf(df_earthquakes, coords = c("Longitude", "Latitude"), crs = 4326)
...
```{r}
plot(df_earthquakes["Magnitude"])
```
```{r}
?leaflet
leaflet(data = df_earthquakes) %>%
    addMarkers(data = st_as_sf(df_earthquakes, coords = c("Longitude", "Latitude"), crs = 4326))
```

```{r}
map <- leaflet(data = df_earthquakes) %>%
  addTiles() %>%  # Add default OpenStreetMap map tiles
  addMarkers(
    lng = ~Longitude, lat = ~Latitude, weight = 1,
    radius = ~Magnitude * 10000, # Adjust the radius based on magnitude for visibility
    color = "#ff0000", fill = TRUE, fillColor = "#ff0000",
